...
var PLAY = 1;
var END = 0;
var beg,eatCoin,eatPallet,death;
...
function preload(){
  ...
  beg=loadSound("Sounds/pacman_beginning.wav")
  eatCoin=loadSound("Sounds/pacman_chomp.wav")
  eatPallet=loadSound("Sounds/pacman_eatfruit.wav")
  death=loadSound("Sounds/pacman_death.wav")
}
...
function draw() {
  ...
  // beg.play();    
    ...
    if(coinGroup.length<=20 && powerpallet.length===0)
    {
      gameState=WON;
    }
...
function lifeCount(){
  for(var c=0;c<ghostGroup.length;c++)
  {
    if(pacman.isTouching(ghostGroup.get(c)))
    {
      ghostGroup.get(c).destroy();
      life = life-1;
    }
  }
